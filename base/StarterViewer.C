...
{
    // Check the depth of the image (number of channels) to decide between RGB and RGBA
    int depth = imgData->getChannels();
    
    if (depth == 3) {
        glDrawPixels(imgData->getWidth(), imgData->getHeight(), GL_RGB, GL_FLOAT, imgData->getData());
    }
    else if (depth == 4) {
        glDrawPixels(imgData->getWidth(), imgData->getHeight(), GL_RGBA, GL_FLOAT, imgData->getData());
    }
    else {
        std::cerr << "Error: Unsupported image depth " << depth << std::endl;
        return;
    }
}
...
void StarterViewer::ApplyLinearFiltering() {
    ...
    img::ImgProc newImage;
    
    img::BoundedLinearConvolution(stencil, *imgData, newImage);

    *imgData = newImage;

    std::cout << "Applied new random stencil to the image." << std::endl;
